...
router.post('/add-funds', async (req, res) => {
    ...
    try {
        ...
        const userId = (await pool.query("SELECT id FROM users WHERE phone_number = $1", [phone_number])).rows[0].id;

        console.log(`Recording transaction for user ID: ${userId}, amount: ${amount}`);
        ...
        res.json({ message: "Funds added successfully", new_balance: result.rows[0].wallet_balance });
    } catch (err) {
...
router.get('/dashboard-stats', async (req, res) => {
  try {
    ...
    `);
    const { today_bids_count, today_bids_amount, today_wins_amount } = todayStatsQuery.rows[0];
...
router.post('/markets/declare-result', async (req, res) => {
...
router.get('/markets/results/:resultId/bids', async (req, res) => {
  ...
  try {
    ...
    const resultData = await pool.query('SELECT * FROM results WHERE id = $1', [resultId]);
    if (resultData.rows.length === 0) return res.status(404).json({ error: "Result not found" });
...
router.put('/settings', upload.single('qr_image'), async (req, res) => {
  ...
  const { 
    upi_id, whatsapp_number, min_amount, existing_qr_url,
    single_digit_rate, jodi_digit_rate, single_panna_rate, double_panna_rate, 
    triple_panna_rate, half_sangam_rate, full_sangam_rate, family_jodi_rate, 
    sp_motor_rate, dp_motor_rate 
  } = req.body;
...
router.get('/markets/:id/liability', async (req, res) => {
  ...
  try {
    ...
    const settingsQuery = await pool.query('SELECT * FROM app_settings WHERE id = 1');
    const settings = settingsQuery.rows[0];

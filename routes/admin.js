...
// Gets ALL non-deleted markets for the admin panel (including paused ones)
router.get("/all", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM markets WHERE (is_deleted = false OR is_deleted IS NULL) ORDER BY id ASC"
    );
  ...
  try {
   ...
   const bidsQuery = `
      SELECT 
        ...
        COALESCE(
          CASE WHEN m.is_deleted = true THEN m.name || ' (Deleted)' ELSE m.name END, 
          'Deleted Market'
        )::text AS market_name
      FROM bids b
      LEFT JOIN markets m ON b.market_id::text = m.id::text
      ...
      SELECT 
        ...
        COALESCE(
          CASE WHEN gm.is_deleted = true THEN gm.name || ' (Deleted)' ELSE gm.name END, 
          'Deleted Market'
        )::text AS market_name
      FROM gali_desawar_bids gb
      LEFT JOIN gali_desawar_markets gm ON gb.market_id::text = gm.id::text
...
router.put("/settings", upload.single("qr_image"), async (req, res) => {
  // 1. Extract standard settings + the 3 Gali Desawar rates
  const {
    upi_id,
    whatsapp_number,
    min_amount,
  ...
  try {
    ...
    await pool.query(
      ...
      SET 
        ...
        dp_motor_rate = $14,
        gd_jodi_rate = $15, gd_haruf_andar_rate = $16, gd_haruf_bahar_rate = $17
      WHERE id = 1
    `,
      [
        upi_id,
